...
int main()
{
	...
	questions.push_back({
		"面積" + to_string(x * y / 2) + "cm^2,底辺" + to_string(y) +
		"cmの三角形の高さを求めよ。",to_string(x) });
	
	//円錐の体積
	x = uniform_int_distribution<>(1, 10)(rand);
	y = uniform_int_distribution<>(1, 5)(rand) * 3;
	questions.push_back({
		"底面の半径" + to_string(x) + "cm,高さ" + to_string(y) +
		"cmの円錐がある\n"  +
		"この円錐の体積をXπcm^3とする。Xの値を求めろ",
		to_string(x * x * y / 3)});
	...
	z = gcd(y + 1, 6);
	questions.push_back({
		"サイコロを一個振って" + to_string(x) + "から" + to_string(x + y)+
		"が出る確率を求めろ",
